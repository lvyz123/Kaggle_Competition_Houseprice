...
lotfrontage = lotfrontage.replace(1e-6,lotfrontage.mean()).map(lambda l: l**1.2)
...
lotarea = train_test_data['LotArea'].map(lambda l: l**0.8)
...
overallqual = train_test_data['OverallQual'].map(lambda o: o**1.2)
sale_housestyle = pd.concat([bldgtype,housestyle,roofstyle,overallqual,train_test_data['OverallCond']],axis=1)
...
bsmtfintype = train_test_data.apply(lambda df: 2*BsmtFinType_Dict[df['BsmtFinType1']]*df['BsmtFinSF1']+2*BsmtFinType_Dict[df['BsmtFinType2']]*df['BsmtFinSF2']+0.5*df['BsmtUnfSF'],axis=1)
...
livarea = train_test_data.apply(lambda df: df['1stFlrSF']**1.05+df['2ndFlrSF']**1.2+df['LowQualFinSF']*0.5,axis=1)
...
garagecars = train_test_data['GarageCars']**2
garageareaspacious = train_test_data.apply((lambda x: x['GarageArea']/avg_area_per_car-x['GarageCars']),axis=1).map(lambda l: l**1.2 if l>=0 else -(-l)**1.2)
...
# Deck and Porch
sale_porch = pd.concat([train_test_data['WoodDeckSF']**2,train_test_data['OpenPorchSF']**1.5,train_test_data['EnclosedPorch'],train_test_data['3SsnPorch'],train_test_data['ScreenPorch']],axis=1)
