...
# Type of Dwelling
sale_subclass = pd.get_dummies(train_test_data['MSSubClass'],prefix='MSSubClass')
...
lotfrontage = lotfrontage.map(lambda l: l/lotfrontage.max())
...
lotarea = train_test_data['LotArea'].map(lambda l: (l-train_test_data['LotArea'].min())/(train_test_data['LotArea'].max()-train_test_data['LotArea'].min()))
...
houseage = houseage.map(lambda l: (l-houseage.min())/(houseage.max()-houseage.min()))
remodage = train_test_data['YearRemodAdd'].map(lambda y: y-train_test_data['YearRemodAdd'].min())
remodage = remodage.map(lambda l: (l-remodage.min())/(remodage.max()-remodage.min()))
...
masvnrarea = masvnrarea.map(lambda m: m/masvnrarea.max())
...
bsmtfintype = bsmtfintype.map(lambda b: b/bsmtfintype.max())
...
fireplace = train_test_data.apply(lambda df: df['Fireplaces']*QuScore_Dict[df['FireplaceQu']]/5,axis=1)
...
livarea = livarea.map(lambda l: (l-livarea.min())/(livarea.max()-livarea.min()))
bedroom = train_test_data['BedroomAbvGr']
...
sale_livrooms = pd.concat([bathnum,livarea,bedroom,kitchen,functional],axis=1)
...
garageage = garageage.map(lambda g: (g-garageage.min())/(garageage.max()-garageage.min()))
...
garagearea = train_test_data['GarageArea'].map(lambda g: g/train_test_data['GarageArea'].max())
garagequalcond = (train_test_data['GarageQual'].map(QuScore_Dict))/5*(train_test_data['GarageCond'].map(QuScore_Dict))/5
paveddrive = train_test_data['PavedDrive'].map(PaveScore_Dict)/2
sale_garage = pd.concat([garagetype,garageage,garagefinish,garagearea,garagequalcond,paveddrive],axis=1)

# Deck and Porch
sale_porch = train_test_data['WoodDeckSF']+train_test_data['OpenPorchSF']+train_test_data['EnclosedPorch']+train_test_data['3SsnPorch']+train_test_data['ScreenPorch']
sale_porch = sale_porch.map(lambda s: s/sale_porch.max())

# Pool
poolarea = train_test_data['PoolArea']/train_test_data['PoolArea'].max()
poolcond = train_test_data['PoolQC'].map(QuScore_Dict)/5
sale_pool = train_test_data['PoolArea']*(train_test_data['PoolQC'].map(QuScore_Dict))/5
sale_pool = sale_pool/sale_pool.max()
